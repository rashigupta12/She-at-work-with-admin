...
// Save step labels shown in the button while saving
...
export function BuilderView({
  ...
  const [bgUploading, setBgUploading] = useState(false);
  const [selectedBackgroundFile, setSelectedBackgroundFile] = useState<File | null>(null);
  const [originalBackgroundFile, setOriginalBackgroundFile] = useState<File | null>(null);
  const [mobilePanel, setMobilePanel] = useState<MobilePanel | null>(null);
  const [dragBgMode, setDragBgMode] = useState(false);
  const [originalImageDimensions, setOriginalImageDimensions] = useState<{ width: number; height: number } | null>(null);
  const [showPreview, setShowPreview] = useState(false);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setCropPendingDims({ width: img.width, height: img.height });
      setOriginalImageDimensions({ width: img.width, height: img.height });
      setOriginalBackgroundFile(file);
  ...
  const handleCropDone = async (crop: CropRegion) => {
    ...
    try {
      ...
      const croppedFile = new File([croppedBlob], srcFile.name, { type: "image/jpeg" });
      const croppedUrl = URL.createObjectURL(croppedBlob);

      URL.revokeObjectURL(srcUrl);

      // ✅ Store the cropped File for Cloudinary upload at save time
      setSelectedBackgroundFile(croppedFile);
      setBanner((b) => ({
        ...b,
        backgroundImageUrl: croppedUrl,   // local preview only
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let cloudinaryUrl: string | null | undefined = banner.backgroundImageUrl;

      if (selectedBackgroundFile) {
        setSaveStep("uploading");
        try {
          cloudinaryUrl = await uploadImageToCloudinary(selectedBackgroundFile, "banner-images");
        } catch (uploadErr: unknown) {
          addToast("error", "Image upload failed: " + ((uploadErr as Error)?.message ?? "Unknown error"));
          setSaveStep("idle");
          return;
        }
      }

      // ── Step 2: Save banner data to backend ────────────────────────────────
      setSaveStep("saving");
      ...
      // ✅ Use Cloudinary URL (or existing URL if no new file) — never send blob:
      const cleanBannerData = {
        ...bannerData,
        elements: cleanElements,
        backgroundImageUrl: cloudinaryUrl?.startsWith("blob:") ? undefined : cloudinaryUrl,
      };

      if (isEdit && initialBanner && "_id" in initialBanner) {
        // ✅ Pass null for file — image already uploaded above
        await BannerAPI.update((initialBanner as Banner)._id, cleanBannerData, null);
        addToast("success", "Banner updated");
        initialRef.current = JSON.stringify(banner);
        setSelectedBackgroundFile(null); // clear after successful save
      } else {
        await BannerAPI.create(cleanBannerData, null);
        addToast("success", "Banner published");
        onBack();
      }
    } catch (err: unknown) {
  ...
  // Save button label
  const saveLabel = () => {
    if (saveStep === "uploading") return "Uploading…";
    if (saveStep === "saving") return "Saving…";
    return isEdit ? "Update" : "Publish";
  };
  ...
  const BgToolbar = () => (
    <div className="bg-white/90 backdrop-blur border-b border-slate-100 flex-shrink-0">
      <div className="flex items-center gap-2 px-4 py-2 overflow-x-auto">
        ...
        {/* ✅ Show "pending upload" badge so user knows image will be uploaded on save */}
        {selectedBackgroundFile && (
          <span className="flex items-center gap-1 text-[10px] font-semibold text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1 flex-shrink-0">
